...
class DiffusionUNet(nn.Module):
    ...
    def __init__(self, hidden_dim=128):
        ...
        self.decoder2 = nn.Linear(hidden_dim, 4)  # output 4D noise (pos+mask)
    ...
    def forward(self, x, t):
        ...
        """
        B, N, in_dim = x.shape  # in_dim=4 if we have (x,y,z,mask)

        # pointnet enc
        x_trans = x.permute(0, 2, 1)  # (B, in_dim, N) where in_dim=4
...
###############################################################
def train_diffusion_model(model, diffusion, dataloader, optimizer, device="cpu", num_epochs=10,
